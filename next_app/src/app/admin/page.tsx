...
export default function UserRegistrationPage() {
  ...
  const { mutate } = useMutation({
    mutationFn: async (data: PostDataType) => {
      ...
      console.log(response);
        ...
        console.error("Error registering user:", error); // エラーをコンソールに表示
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["users"] });
  ...
  const isButtonDisabled = name === "" || password === ""; // ユーザー名またはパスワードが空ならボタンを無効化
  ...
  // 登録ボタンを押すとユーザーが登録される
  const handleClick = () => {
    mutate({ name: name, password: password });
    setUserName(""); // ユーザー名をクリア
  ...
  return (
    <>
      <Header title="ユーザー登録ページ" icon={<FaUserCog size={30} />} />
      {isLoading ? <CustomLinearProgress /> : <div></div>}
      <h2 className="text-xl text-center my-4">ユーザー一覧</h2>
      <div style={tableContainerStyles}>
        <table {...getTableProps()} style={tableStyles}>
          ...
          <tbody {...getTableBodyProps()}>
            {page.map((row: Row<User>, index: number) => {
              ...
              return (
                <tr {...row.getRowProps()} key={index}>
                  {row.cells.map((cell) => (
                    <td {...cell.getCellProps()} style={tdStyles} key={cell.column.id}>
                      {cell.render("Cell")}
                    </td>
                  ))}
                </tr>
      ...
      <div className="text-center justify-center my-8">
        ...
        <input
          ...
          onChange={handleNameChange}
        />
        <h3 className="text-lg text-center my-2">パスワード</h3>
        <input
          type="password"
          className="border p-2 rounded-md"
          value={password}
